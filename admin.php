...
if (array_key_exists("$session_prefix-logged_in", $_SESSION)) { // logged in to admin page
    ...
    if (array_key_exists("action", $_POST)) {
        ...
        // Cleanup the DB
        $sql->exec("DELETE FROM Faucetinabox_Addresses WHERE last_used<DATE_SUB(NOW(), INTERVAL 90 DAY);");
        $sql->exec("DELETE FROM Faucetinabox_IPs WHERE last_used<DATE_SUB(NOW(), INTERVAL 30 DAY);");
        $q = $sql->prepare("SELECT id, address FROM Faucetinabox_Refs;");
        $q->execute(array());
        while ($result=$q->fetch()) {
            $q2 = $sql->prepare("SELECT address FROM Faucetinabox_Addresses WHERE ref_id=? LIMIT 1;");
            $q2->execute(array($result['id']));
            if (!$result2=$q2->fetch()) {
                $q3 = $sql->prepare("DELETE FROM Faucetinabox_Refs WHERE id=?;");
