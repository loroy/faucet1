...
// Anti Bot Links 6.00 pro
// by bit.makejar.com
// working demo at http://faucetboy.xyz/
...
class antibotlinks {
    ...
    var $use_gd=true;
    var $fonts=array();
    var $abl_settings=array();

    public function __construct($use_gd=true, $font_type='') {
        global $sql;
        // check if abl already installed
        $abl_settings_array = $sql->query("SELECT `name`, `value` FROM `Faucetinabox_Settings` WHERE `name` LIKE 'abl_%';")->fetchAll(PDO::FETCH_ASSOC);
        foreach ($abl_settings_array as $k=>$v) {
            $this->abl_settings[$v['name']]=$v['value'];
        }
        //
        if (count($this->abl_settings)==0) {
            $sql->exec("INSERT INTO Faucetinabox_Settings SET `name`='abl_enabled', `value`='off';");
            $sql->exec("INSERT INTO Faucetinabox_Settings SET `name`='abl_light_colors', `value`='off';");
            $sql->exec("INSERT INTO Faucetinabox_Settings SET `name`='abl_background', `value`='off';");
            $sql->exec("INSERT INTO Faucetinabox_Settings SET `name`='abl_noise', `value`='off';");
            $sql->exec("INSERT INTO Faucetinabox_Settings SET `name`='abl_universe', `value`='one=>1, two=>2, three=>3, four=>4, five=>5, six=>6, seven=>7, eight=>8, nine=>9, ten=>10\n\n1=>one, 2=>two, 3=>three, 4=>four, 5=>five, 6=>six, 7=>seven, 8=>eight, 9=>nine, 10=>ten\n\n1=>I, 2=>II, 3=>III, 4=>IV, 5=>V, 6=>VI, 7=>VII, 8=>VIII, 9=>IX, 10=>X\n\ncat=>C@t, dog=>d0g, lion=>1!0n, tiger=>T!g3r, monkey=>m0nk3y, elephant=>31eph@nt, cow=>c0w, fox=>f0x, mouse=>m0us3, ant=>@nt\n\n2-1=>1, 1+1=>2, 1+2=>3, 2+2=>4, 3+2=>5, 2+4=>6, 3+4=>7, 4+4=>8, 1+8=>9, 5+6=>11\n\n1=>3-2, 2=>8-6, 3=>1+2, 4=>3+1, 5=>9-4, 6=>3+3, 7=>6+1, 8=>2*4, 9=>3+6, 10=>2+8\n\n--x=>OOX, -x-=>OXO, x--=>XOO, xx-=>XXO, -xx=>OXX, x-x=>XOX, ---=>OOO, xxx=>XXX, x-x-=>XOXO, -x-x=>OXOX\n\n--x=>--+, -x-=>-+-, x--=>+--, xx-=>++-, -xx=>-++, x-x=>+-+, ---=>---, xxx=>+++, x-x-=>+-+-, -x-x=>-+-+\n\n--x=>oo+, -x-=>o+o, x--=>+oo, xx-=>++o, -xx=>o++, x-x=>+o+, ---=>ooo, xxx=>+++, x-x-=>+o+o, -x-x=>o+o+\n\noox=>--+, oxo=>-+-, xoo=>+--, xxo=>++-, oxx=>-++, xox=>+-+, ooo=>---, xxx=>+++, xoxo=>+-+-, oxox=>-+-+\n\n2*A=>AA, 3*A=>AAA, 2*B=>BB, 3*B=>BBB, 1*A+1*B=>AB, 1*A+2*B=>ABB, 2*A+2*B=>AABB, 2*C=>CC, 3*C=>CCC, 1*C+1*A=>CA, 1*C+1*B=>CB, 1*C+2*A=>CAA, 1*C+2*B=>CBB, 2*C+1*A=>CCA\n\nAA=>2*A, AAA=>3*A, BB=>2*B, BBB=>3*B, AB=>1*A+1*B, ABB=>1*A+2*B, AABB=>2*A+2*B, CC=>2*C, CCC=>3*C, CA=>1*C+1*A, CB=>1*C+1*B, CAA=>1*C+2*A, CBB=>1*C+2*B, CCA=>2*C+1*A\n\nzoo=>200, ozo=>020, ooz=>002, soo=>500, oso=>050, oos=>005, lol=>101, sos=>505, zoz=>202, lll=>111';");
            $sql->exec("CREATE TABLE `Faucetinabox_ABL_Log` (
                                        `Faucetinabox_ABL_Log_id` int(11) NOT NULL AUTO_INCREMENT,
                                        `Faucetinabox_ABL_Log_time` int(11) NOT NULL DEFAULT '0',
                                        `Faucetinabox_ABL_Log_IP` varchar(50) NOT NULL DEFAULT '',
                                        `Faucetinabox_ABL_Log_address` varchar(50) NOT NULL DEFAULT '',
                                        `Faucetinabox_ABL_Log_address_ref` varchar(50) NOT NULL DEFAULT '',
                                        `Faucetinabox_ABL_Log_status` enum('valid','invalid','possibly bot') NOT NULL DEFAULT 'invalid',
                                        `Faucetinabox_ABL_Log_session_id` varchar(50) NOT NULL DEFAULT '',
                                        PRIMARY KEY (`Faucetinabox_ABL_Log_id`),
                                        KEY `Faucetinabox_ABL_Log_time` (`Faucetinabox_ABL_Log_time`),
                                        KEY `Faucetinabox_ABL_Log_session_id` (`Faucetinabox_ABL_Log_session_id`),
                                        KEY `Faucetinabox_ABL_Log_IP` (`Faucetinabox_ABL_Log_IP`)
                                    );");
    ...
    public function generate($link_count=3, $force_regeneration=false) {
        ...
        $q=$sql->prepare("SELECT count(Faucetinabox_ABL_Log_id) as count_ABL_Log_id
                                      FROM `Faucetinabox_ABL_Log`
                                      WHERE
                                        Faucetinabox_ABL_Log_IP=?
                                      AND
                                        Faucetinabox_ABL_Log_status='invalid'
                                      AND
                                        Faucetinabox_ABL_Log_time>?
                                     ;");
        $q->execute(array(trim(getIP()), time()-600));
        if ($q_row=$q->fetch(PDO::FETCH_ASSOC)) {
...
<?php
    ...
    public function is_valid($record_in_db=true) {
        ...
        if ((is_array($_POST))&&(count($_POST)>0)&&($record_in_db)) {
            ...
            $q=$sql->prepare("INSERT INTO `Faucetinabox_ABL_Log` SET
                                                        Faucetinabox_ABL_Log_time=?,
                                                        Faucetinabox_ABL_Log_IP=?,
                                                        Faucetinabox_ABL_Log_address=?,
                                                        Faucetinabox_ABL_Log_address_ref=?,
                                                        Faucetinabox_ABL_Log_status=?,
                                                        Faucetinabox_ABL_Log_session_id=?
                                             ;");
            $q->execute(array(
                                                    time(),
                                                    trim(getIP()),
            ...
            if (mt_rand(0, 20)==5) {
                $sql->exec("DELETE FROM `Faucetinabox_ABL_Log` WHERE Faucetinabox_ABL_Log_time<".(time()-86400).";");
            }
        }
        //
        return $_SESSION['antibotlinks']['valid'];
            ...
            switch ($_POST['action']) {
                case 'ajax_abl_get_claim_log':
                    ...
                    $abl_log_query=$sql->query("SELECT
                                                      Faucetinabox_ABL_Log_id as ABL_Log_id,
                                                      Faucetinabox_ABL_Log_time as ABL_Log_time,
                                                      Faucetinabox_ABL_Log_IP as ABL_Log_IP,
                                                      Faucetinabox_ABL_Log_address as ABL_Log_address,
                                                      Faucetinabox_ABL_Log_address_ref as ABL_Log_address_ref,
                                                      Faucetinabox_ABL_Log_status as ABL_Log_status
                                                    FROM
                                                      Faucetinabox_ABL_Log
                                                    WHERE
                                                      Faucetinabox_ABL_Log_id>".(int)$last_id."
                                                    AND
                                                      Faucetinabox_ABL_Log_time>".(time()-86400)."
                                                    ORDER BY
                                                      Faucetinabox_ABL_Log_id
                                                    DESC
                                                    LIMIT 2500
                                                    ;");
                        while ($abl_log_row=$abl_log_query->fetch(PDO::FETCH_ASSOC)) {
    ...
    public function admin_config() {
        ...
        foreach ($this->abl_settings as $abl_settings_k=>$abl_settings_v) {
            if(in_array($abl_settings_k, ['abl_enabled', 'abl_light_colors', 'abl_background', 'abl_noise'])) {
                $abl_settings_v = $abl_settings_v == 'on' ? 'checked' : '';
            }
            $page = str_replace('<:: '.$abl_settings_k.' ::>', $abl_settings_v, $page);
        }
...
function abl_claim_loop() {
    $.post(\''.basename($_SERVER['PHP_SELF']).'\', {action:\'ajax_abl_get_claim_log\', last_id:abl_claim_last_id, csrftoken:\''.$_SESSION["$session_prefix-csrftoken"].'\'})
